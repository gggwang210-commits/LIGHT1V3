...
LIGHTONE V3는 PT샵·헬스장·필라테스 센터가 회원의 운동 수행 기록, 통증 반응 참고값, RPE, 촬영 QC, 트레이너 메모를 구조화해 상담 리포트 초안을 만드는 Django 기반 MVP입니다.

이 프로젝트는 의료 서비스가 아닙니다. AI 또는 점수 계산 로직은 트레이너의 판단을 대체하지 않으며, 회원에게 전달되는 모든 리포트는 담당 트레이너가 검토해야 합니다.

## 1. 프로젝트 개요

LIGHTONE V3의 목적은 트레이너가 흩어진 운동상담 기록을 정리하고, 재등록 상담이나 컨디셔닝 안내에 활용할 수 있는 설명 가능한 리포트 초안을 만드는 것입니다.
...
현재 실행 기준은 `lightone_v2_django/`입니다. 업로드된 이전 산출물은 삭제하지 않고 `legacy/`, `legacy-docs/`, `lightone-main/`, `lightone_django/`, `legacy-django-complete/` 등에 보존되어 있습니다.

| 경로 | 역할 |
|------|------|
| `lightone_v2_django/` | 현재 실행 기준 Django MVP |
| `docs/` | 최신 사업·제품·운영 문서 허브 |
| `sample_data/` | 합성 샘플 데이터 |
| `prototype/` | 정적 대시보드 프로토타입 |
| `legacy/`, `legacy-docs/`, `lightone-main/` | 이전 버전과 참고 자료 보관 |
| `.github/workflows/` | 활성 GitHub Actions CI |
| `docs/archive/` | 실행 대상에서 제외한 보존 자료 |

자세한 기준은 `docs/repository-map.md`를 확인하세요.
...
- 트레이너 대시보드
- 상담 리포트 상세 화면
- QS/JATC 참고 엔진: `lightone_v2_django/lightone/algorithms.py`
- QS 라우팅 유틸: `lightone_v2_django/lightone/utils/qs_calculator.py`
- 합성 데이터 생성 명령
- 비의료 안전 문구 테스트
- 개인정보·배포 전 체크리스트
- 파일럿 검증 문서
...
QS/JATC 엔진은 `lightone_v2_django/lightone/algorithms.py`와 `lightone_v2_django/lightone/utils/qs_calculator.py`에 있습니다.
